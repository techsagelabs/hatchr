-- SAFE Real-time Optimization for Hatchr
-- This version checks existing publications and only adds missing tables
...
-- Notifications indexes (using correct column name 'user_id')
...
-- 2. SAFELY ENABLE REAL-TIME for tables (only if not already enabled)
-- This checks existing publication and only adds missing tables
...
BEGIN
    ...
    FOR table_name IN VALUES ('projects'), ('votes'), ('comments'), ('connections'), ('notifications'), ('user_profiles')
    LOOP
...
SELECT 'Real-time publications updated safely ✅' as status;
...
-- Create triggers for vote count updates (safe to recreate)
...
SELECT 'Vote count triggers created successfully ✅' as status;
...
-- Create triggers for comment count updates (safe to recreate)
...
SELECT 'Comment count triggers created successfully ✅' as status;

-- 6. VERIFY real-time setup and show current status
SELECT '🎉 REAL-TIME OPTIMIZATION COMPLETE! 🎉' as status;

-- Show which tables are currently enabled for real-time
SELECT 
    '📊 Real-time enabled tables:' as info,
    schemaname, 
    tablename 
...
AND tablename IN ('projects', 'votes', 'comments', 'connections', 'notifications', 'user_profiles')
...
-- Show triggers
SELECT 
    '⚡ Real-time triggers active:' as info,
    trigger_name,
    event_object_table
FROM information_schema.triggers
WHERE trigger_schema = 'public'
AND trigger_name LIKE '%project%count%'
ORDER BY event_object_table, trigger_name;

SELECT 'Setup verification complete! Your real-time features are now optimized! 🚀' as final_status;
